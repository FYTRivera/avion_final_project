class BookedSessionMailer < ApplicationMailer
  ...
  def booked_session
    ...
    @greeting = "Hi, you have applied for a session with Dr. #{@doctor.first_name} #{@doctor.last_name} from #{@meeting.start_time.strftime('%Y-%m-%d | %I:%M %p')} to #{@meeting.end_time.strftime('%Y-%m-%d | %I:%M %p')}"
